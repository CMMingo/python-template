...
This skill is self-contained -- it doesn't rely on `implement` or any other skill knowing it exists, since UI work is occasional here, not the main track. Once the design is built and matches the reviewed plan from the Restraint and self-critique step:

- Dispatch the `code-reviewer` subagent for an independent, fresh-eyes pass over the diff. Tell it explicitly, in the dispatch prompt, that this is UI work and to also check it against this skill's own principles (genericness/templated tells, the quality floor: responsive down to mobile, visible keyboard focus, reduced motion respected) -- `code-reviewer` doesn't carry frontend-specific criteria itself, so hand it the file path (`.claude/skills/frontend-design/SKILL.md`) and ask it to weigh the diff against it, on top of its normal correctness/security pass.
- If this UI work is implementing a seam from an existing `/spec` (i.e. you were reached from `implement`), also dispatch `logic-checker` against that seam, same as any other seam would get -- tell it the same way to check the aesthetic/UX decisions against this skill's principles, not just the spec's prose, since a passing test rarely pins down palette or layout.
- If it's standalone UI work with no spec behind it, skip `logic-checker` -- there's no spec decision for it to check against -- and rely on `code-reviewer` plus your own critique pass.
- Once reviewed, tell the user it's ready for `/commit-push`. Don't commit or push yourself -- same convention as everywhere else in this template.
